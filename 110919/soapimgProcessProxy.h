	...
	{ soap = soap_new(); endpoint = "http://localhost/cgi-bin/imgProcessServer"; if (soap && !soap->namespaces) { static const struct Namespace namespaces[] = 
{
	...
	/// Invoke 'viewResult' of service 'imgProcess' and return error code (or SOAP_OK)
	virtual int ns__viewResult(char *src, struct ns__signalResponse *out) { return soap ? soap_call_ns__viewResult(soap, endpoint, NULL, src, out) : SOAP_EOM; };
};
#endif

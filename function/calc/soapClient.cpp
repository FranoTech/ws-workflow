...
SOAP_SOURCE_STAMP("@(#) soapClient.cpp ver 2.8.4 2011-10-25 07:13:04 GMT")
...
{	struct ns__test soap_tmp_ns__test;
	...
	if (!soap_endpoint)
		soap_endpoint = "http://localhost/cgi-bin/calcserver";
	soap->encodingStyle = "";
	soap_tmp_ns__test.s = s;
	soap_tmp_ns__test.i = i;
	soap_begin(soap);
	soap_serializeheader(soap);
	...
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || soap_put_ns__test(soap, &soap_tmp_ns__test, "ns:test", NULL)
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	}
	...
	*t = NULL;
	...
	if (soap_recv_fault(soap, 1))
		return soap->error;
	soap_tmp_ns__testResponse = soap_get_ns__testResponse(soap, NULL, "", "");
	if (soap->error)
		return soap_recv_fault(soap, 0);
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap_closesock(soap);
	if (t && soap_tmp_ns__testResponse->t)
		*t = *soap_tmp_ns__testResponse->t;
	return soap_closesock(soap);
}

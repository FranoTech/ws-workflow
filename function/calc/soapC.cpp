...
SOAP_SOURCE_STAMP("@(#) soapC.cpp ver 2.8.4 2011-10-25 07:13:04 GMT")
...
{	switch (p->type)
	...
	case SOAP_TYPE_ns__testResponse:
		if (p->size < 0)
			SOAP_DELETE((struct ns__testResponse*)p->ptr);
		else
			SOAP_DELETE_ARRAY((struct ns__testResponse*)p->ptr);
		break;
	case SOAP_TYPE_ns__test:
		if (p->size < 0)
			SOAP_DELETE((struct ns__test*)p->ptr);
		else
			SOAP_DELETE_ARRAY((struct ns__test*)p->ptr);
		break;
#ifndef WITH_NOGLOBAL
	case SOAP_TYPE_SOAP_ENV__Header:
		if (p->size < 0)
...
{
	(void)soap; (void)a; /* appease -Wall -Werror */
	soap_serialize_string(soap, &a->s);
}
...
{	(void)soap; (void)tag; (void)id; (void)type;
	if (soap_element_begin_out(soap, tag, soap_embedded_id(soap, id, a, SOAP_TYPE_ns__test), type))
		return soap->error;
	if (soap_out_string(soap, "s", -1, &a->s, ""))
		return soap->error;
	...
	if (soap->body && !*soap->href)
	{
		...
		{	soap->error = SOAP_TAG_MISMATCH;
			if (soap_flag_s && (soap->error == SOAP_TAG_MISMATCH || soap->error == SOAP_NO_TAG))
				if (soap_in_string(soap, "s", &a->s, "xsd:string"))
				{	soap_flag_s--;
					continue;
				}
...
SOAP_FMAC3 int SOAP_FMAC4 soap_put_ns__test(struct soap *soap, const struct ns__test *a, const char *tag, const char *type)
{
	...
	if (soap_out_ns__test(soap, tag?tag:"ns:test", id, a, type))
		return soap->error;
	return soap_putindependent(soap);
}
...
SOAP_FMAC1 struct ns__test * SOAP_FMAC2 soap_instantiate_ns__test(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (!cp)
		return NULL;
	if (n < 0)
	{	cp->ptr = (void*)SOAP_NEW(struct ns__test);
		if (size)
			*size = sizeof(struct ns__test);
	}
	else
	{	cp->ptr = (void*)SOAP_NEW(struct ns__test[n]);
		if (!cp->ptr)
		{	soap->error = SOAP_EOM;
			return NULL;
		}
...
{
	(void)soap; (void)a; /* appease -Wall -Werror */
	soap_serialize_PointerTostring(soap, &a->t);
}
...
{	(void)soap; (void)tag; (void)id; (void)type;
	if (soap_element_begin_out(soap, tag, soap_embedded_id(soap, id, a, SOAP_TYPE_ns__testResponse), type))
		return soap->error;
	if (soap_out_PointerTostring(soap, "t", -1, &a->t, ""))
		return soap->error;
	return soap_element_end_out(soap, tag);
}
	...
	if (soap->body && !*soap->href)
	{
		...
		{	soap->error = SOAP_TAG_MISMATCH;
			if (soap_flag_t && soap->error == SOAP_TAG_MISMATCH)
				if (soap_in_PointerTostring(soap, "t", &a->t, "xsd:string"))
				{	soap_flag_t--;
					continue;
				}
			if (soap->error == SOAP_TAG_MISMATCH)
...
SOAP_FMAC3 int SOAP_FMAC4 soap_put_ns__testResponse(struct soap *soap, const struct ns__testResponse *a, const char *tag, const char *type)
{
	...
	if (soap_out_ns__testResponse(soap, tag?tag:"ns:testResponse", id, a, type))
		return soap->error;
	return soap_putindependent(soap);
}
...
SOAP_FMAC1 struct ns__testResponse * SOAP_FMAC2 soap_instantiate_ns__testResponse(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (!cp)
		return NULL;
	if (n < 0)
	{	cp->ptr = (void*)SOAP_NEW(struct ns__testResponse);
		if (size)
			*size = sizeof(struct ns__testResponse);
	}
	else
	{	cp->ptr = (void*)SOAP_NEW(struct ns__testResponse[n]);
		if (!cp->ptr)
		{	soap->error = SOAP_EOM;
			return NULL;
		}

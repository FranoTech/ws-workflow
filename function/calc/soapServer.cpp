...
SOAP_SOURCE_STAMP("@(#) soapServer.cpp ver 2.8.4 2011-10-25 07:13:04 GMT")
...
{	struct ns__test soap_tmp_ns__test;
	...
	soap_tmp_ns__testResponse.t = &soap_tmp_string;
	...
	if (soap_body_end_in(soap)
	 ...
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = ns__test(soap, soap_tmp_ns__test.s, soap_tmp_ns__test.i, soap_tmp_ns__testResponse.t);
	if (soap->error)
		return soap->error;
	soap_serializeheader(soap);
	...
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || soap_put_ns__testResponse(soap, &soap_tmp_ns__testResponse, "ns:testResponse", NULL)
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	};

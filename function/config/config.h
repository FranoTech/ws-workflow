...
private:
	...
	void extractContents(const std::string &line) 
	{
		...
		if (!keyExists(key))
			...
			std::cerr << "CFG: Can only have unique key names!\n" ;
	}

	void parseLine(const std::string &line, size_t const lineNo)
	{
		if (line.find('=') == line.npos)
			std::cerr << "CFG: Couldn't find separator on line: " + T_to_string(lineNo) + "\n" ;

		if (!validLine(line))
			std::cerr << "CFG: Bad format for line: " + T_to_string(lineNo) + "\n" ;
	...
	void ExtractKeys()
	{
		...
		if (!file)
			std::cerr <<  "CFG: File " + fName + " couldn't be found!\n" ;
...
public:
	...
	static T string_to_T(std::string const &val) 
	{
		...
		T returnVal;
		if (!(istr >> returnVal))
			std::cerr << "CFG: Not a valid " + (std::string)typeid(T).name() + " received!\n" ;

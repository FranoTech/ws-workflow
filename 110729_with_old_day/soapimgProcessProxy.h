	...
	{ soap = soap_new(); endpoint = "http://localhost/cgi-bin/imgProcessServer"; if (soap && !soap->namespaces) { static const struct Namespace namespaces[] = 
{
	...
	virtual int ns__Ipl1ChToMat(char *InputFilename, char *&OutputFilename) { return soap ? soap_call_ns__Ipl1ChToMat(soap, endpoint, NULL, InputFilename, OutputFilename) : SOAP_EOM; };
	/// Invoke 'MatToIpl1Ch' of service 'imgProcess' and return error code (or SOAP_OK)
	virtual int ns__MatToIpl1Ch(char *InputFilename, char *&OutputFilename) { return soap ? soap_call_ns__MatToIpl1Ch(soap, endpoint, NULL, InputFilename, OutputFilename) : SOAP_EOM; };
	/// Invoke 'BinaryThreshold' of service 'imgProcess' and return error code (or SOAP_OK)
	virtual int ns__BinaryThreshold(char *InputFilename, double threshold, double maxValue, char *&OutputFilename) { return soap ? soap_call_ns__BinaryThreshold(soap, endpoint, NULL, InputFilename, threshold, maxValue, OutputFilename) : SOAP_EOM; };
	/// Invoke 'MorphOpen' of service 'imgProcess' and return error code (or SOAP_OK)
	virtual int ns__MorphOpen(char *InputFilename, char *&OutputFilename) { return soap ? soap_call_ns__MorphOpen(soap, endpoint, NULL, InputFilename, OutputFilename) : SOAP_EOM; };
};
#endif

	...
	{ soap = soap_new(); endpoint = "http://localhost/cgi-bin/imgProcessServer"; if (soap && !soap->namespaces) { static const struct Namespace namespaces[] = 
{
	...
	virtual int ns__BinaryThreshold(int sharedKey, int imgHeight, int imgWidth, double threshold, double maxValue, ns__ImageData &out) { return soap ? soap_call_ns__BinaryThreshold(soap, endpoint, NULL, sharedKey, imgHeight, imgWidth, threshold, maxValue, out) : SOAP_EOM; };
	/// Invoke 'MorphOpen' of service 'imgProcess' and return error code (or SOAP_OK)
	virtual int ns__MorphOpen(int sharedKey, int imgHeight, int imgWidth, ns__ImageData &out) { return soap ? soap_call_ns__MorphOpen(soap, endpoint, NULL, sharedKey, imgHeight, imgWidth, out) : SOAP_EOM; };
	/// Invoke 'MatToJPG' of service 'imgProcess' and return error code (or SOAP_OK)
	virtual int ns__MatToJPG(int sharedKey, int imgHeight, int imgWidth, char *&OutputFilename) { return soap ? soap_call_ns__MatToJPG(soap, endpoint, NULL, sharedKey, imgHeight, imgWidth, OutputFilename) : SOAP_EOM; };
};
#endif

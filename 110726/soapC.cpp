...
SOAP_SOURCE_STAMP("@(#) soapC.cpp ver 2.7.17 2011-07-27 15:45:23 GMT")
...
{
	(void)soap; /* appease -Wall -Werror */
	soap_default_int(soap, &this->ns__ImageData::sharedKey);
	soap_default_int(soap, &this->ns__ImageData::imgHeight);
	soap_default_int(soap, &this->ns__ImageData::imgWidth);
}
...
SOAP_FMAC3 int SOAP_FMAC4 soap_out_ns__ImageData(struct soap *soap, const char *tag, int id, const ns__ImageData *a, const char *type)
{
	if (soap_element_begin_out(soap, tag, soap_embedded_id(soap, id, a, SOAP_TYPE_ns__ImageData), type))
		return soap->error;
	if (soap_out_int(soap, "sharedKey", -1, &(a->ns__ImageData::sharedKey), ""))
		return soap->error;
	if (soap_out_int(soap, "imgHeight", -1, &(a->ns__ImageData::imgHeight), ""))
		return soap->error;
	if (soap_out_int(soap, "imgWidth", -1, &(a->ns__ImageData::imgWidth), ""))
		return soap->error;
...
SOAP_FMAC3 ns__ImageData * SOAP_FMAC4 soap_in_ns__ImageData(struct soap *soap, const char *tag, ns__ImageData *a, const char *type)
{
	...
	size_t soap_flag_sharedKey1 = 1;
	size_t soap_flag_imgHeight1 = 1;
	size_t soap_flag_imgWidth1 = 1;
	if (soap->body && !*soap->href)
	{
		for (;;)
		{	soap->error = SOAP_TAG_MISMATCH;
			if (soap_flag_sharedKey1 && soap->error == SOAP_TAG_MISMATCH)
				if (soap_in_int(soap, "sharedKey", &(a->ns__ImageData::sharedKey), "xsd:int"))
				{	soap_flag_sharedKey1--;
					continue;
				}
			if (soap_flag_imgHeight1 && soap->error == SOAP_TAG_MISMATCH)
				if (soap_in_int(soap, "imgHeight", &(a->ns__ImageData::imgHeight), "xsd:int"))
				{	soap_flag_imgHeight1--;
					continue;
				}
			if (soap_flag_imgWidth1 && soap->error == SOAP_TAG_MISMATCH)
				if (soap_in_int(soap, "imgWidth", &(a->ns__ImageData::imgWidth), "xsd:int"))
				{	soap_flag_imgWidth1--;
					continue;
				}
	...
	if ((soap->mode & SOAP_XML_STRICT) && (soap_flag_sharedKey1 > 0 || soap_flag_imgHeight1 > 0 || soap_flag_imgWidth1 > 0))
